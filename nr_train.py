...
# if more than one tfrecord file you have generated, you can shuffle them to get higher randomness数据随机重排
...
def cv_acc_of_early_stop(batch_size, num_samples, sess):
    ...
    for step in range(num_batchs):
        ...
        cv_accuracy_update_value = sess.run(cv_accuracy,
                                            feed_dict={IMGS: imgs, Labels: labels, keep_prob: 1.0})
        accuracy_count += 1
...
if __name__ == '__main__':
    ...
    tfrecord_path = r'./cv_tfrecord/cv_my_data.tfrecord'
    cv_dataset = tf.data.TFRecordDataset([tfrecord_path])
    ...
    with slim.arg_scope(DNNs_arg_scope()):
        logits, end_points = DNNs(IMGS, num_classes=num_class, keep_prob=keep_prob, is_training=True)
